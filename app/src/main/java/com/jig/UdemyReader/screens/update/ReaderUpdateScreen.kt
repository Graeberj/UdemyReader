...
import com.jig.UdemyReader.components.*
...
import com.jig.UdemyReader.utils.formatDate
        ...
        Surface(modifier = Modifier
            ...
            verticalArrangement = Arrangement.Top,
                ...
                if (bookInfo.loading == true){
                    ...
                    shape = CircleShape,
                    elevation = 4.dp){
                        ShowUpdate(bookInfo = viewModel.data.value, bookItemId = bookItemId)
...
@Composable
fun ShowUpdate(bookInfo: DataOrException<List<MBook>,
        Boolean,
        Exception>,
               bookItemId: String
) {
...
fun ShowSimpleForm(book: MBook, navController: NavController) {
    ...
    SimpleForm(defaultValue = book.notes.toString().ifEmpty { "Nothing to see here" }
    ){note ->
    ...
    TextButton(onClick = { isFinishedReading.value = true },
        ...
        if (book.finishedReading == null){
            if(!isFinishedReading.value){
                ...
                Text(text = "book complete")
            }
        } else {
            Text(text = "book completed on ${formatDate(book.finishedReading!!)}")
        }
    }
